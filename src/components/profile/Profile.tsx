...
export default function Profile({ email }: ProfileProps) {
  const { data: profile, isLoading, error } = usePublicProfile(email, !!email);
  ...
  if (!profile) {
    return (
      <div className="p-4 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
        <p className="text-gray-600 dark:text-gray-400 text-center">No profile data available</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">Complete user information and activity</p>
        </div>
      ...
      {profile.subscription && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Plan</label>
              <p className="text-gray-900 dark:text-white">{profile.subscription.plan.name}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Status</label>
              <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
            ...
            <div>
              ...
              <p className="text-gray-900 dark:text-white">
                ${profile.subscription.plan.price}/{profile.subscription.plan.billingCycle}
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Auto Renew</label>
              <span className={`inline-flex items-center gap-1 ${
                profile.subscription.autoRenew ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
              }`}>
                {profile.subscription.autoRenew ? '✅ Yes' : '❌ No'}
              </span>
            </div>
          </div>
        </div>
      ...
      {profile.recentLogins && profile.recentLogins.length > 0 && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
          ...
          <div className="space-y-3">
            {profile.recentLogins.map((log: any, index: number) => (
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
          ...
          <div className="space-y-3">
            {profile.activeSessions.map((session: any, index: number) => (
              <div key={session.id || index} className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div className="flex items-center justify-between mb-2">
